function DrawStreamGraph(skillsHash, graphStartYear, svgName, height, colorSchema) {
  ...
  var skillsData = Object.keys(skillsHash).map(function(skillKey) {
    return skillsHash[skillKey];
  });
  ...
  var svg = d3.select(svgName)
    ...
    .attr("viewBox", "0 0 " + (containerWidth * 0.96) + " " + height);
  ...
  var yMax = d3.max(stackedData, function(skill) {
    return d3.max(skill.values, function(month) {
  ...
  var xAxis = d3.svg.axis()
    ...
    .tickFormat(function(d) {
      return " " + (graphStartYear + Math.floor(d / 12));
    });
